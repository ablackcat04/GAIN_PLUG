...
GainPlugAudioProcessor::GainPlugAudioProcessor()
     ...
     : AudioProcessor (BusesProperties()
                       ...
                       treeState(*this, nullptr, "Parameters", { std::make_unique<juce::AudioParameterFloat>("Gain", "gain", -48.0f, 48.0f, 0.0f)})
                       
    
#endif
{
    juce::NormalisableRange<float> gainRange(0.0f, 1.0f);
    juce::AudioProcessorParameter::Category gainCategory = juce::AudioProcessorParameter::Category::outputGain;
    treeState.createAndAddParameter("Gain", "gain", "gain", gainRange, 0.5f, nullptr, nullptr, false, true, false, gainCategory, false);
...
void GainPlugAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        if (CurrentBypassState == GainPlugAudioProcessor::BypassState::normal)
        {
            for (int sample = 0; sample < buffer.getNumSamples(); sample++)
            {
                GainPlugAudioProcessor::temp = *(buffer.getReadPointer(channel, sample));
                buffer.getWritePointer(channel)[sample] = coefficient * GainPlugAudioProcessor::temp;
            }
        }
